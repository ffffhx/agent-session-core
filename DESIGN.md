...
3. **接 agent-retro**（下一步）：改调本包、加 history.jsonl 趋势 + 成本口径 + 行动清单，首次 commit。
4. **接 token-board**：collector/agent 改调本包，删三套重复解析；parity diff 对齐。
5. **snapshot 投影 + privacy 抽出 + 接 codex-snapshots**：拆 3079 行大文件。
6. **claude-hud**：改读统一数据。
