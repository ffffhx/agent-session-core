...
1. ✅ **core + token 投影**（本次）：在真实日志上验证 reset-aware 正确性。
2. **接 token-board**：collector/agent 改调本包，删三套重复；parity diff 对齐。
3. **metrics 投影 + 接 agent-retro**：顺手修 duration/model/compaction，首次 commit agent-retro。
4. **snapshot 投影 + privacy 抽出 + 接 codex-snapshots**：拆 3079 行大文件。
5. **claude-hud**：改读统一数据。
